...
function eliminarProductoProducto($idProducto){
	global $bd;
	$query="Select * From usuario where UserName like '".$_SESSION['UserName']."'"; 
	$resuser = $bd->seleccionar($query);
	$res=$resuser->fetch_assoc();
	$id=$res['idUsuario'];
	$sql="UPDATE cesta_lineas SET cantidad=0 WHERE idCesta =" .$id .  
		" AND IdProducto=".$idProducto;
   $bd->eliminarProductoProducto($sql);
}

if (isset($_GET['eliminarProductoProducto'])) {
   eliminarProductoProducto($_GET['eliminarProductoProducto']);
}
...
function actualizarProducto($idProducto, $cantidad){
  ...
  $res=$resuser->fetch_assoc();
  $id=$res['idUsuario'];
  $sql= "UPDATE cesta_lineas SET cantidad=".$cantidad." WHERE idCesta =" . $id . " AND IdProducto=".$idProducto;
  $bd->actualizarProducto($sql);
}

if (isset($_GET['actualizarProducto'])){
  $idProducto=$_GET['actualizarProducto'];
  $cantidad=$_GET['cantidad'];
  actualizarProducto($idProducto,$cantidad);
}
...
<body>
	<div class="grid-container">
		...
		<div class="grid-item1"><H1>Producto - <?php echo $_GET['nombreTipoProducto']; ?></H1>
		</div>
		...
		<div class="grid-container2">
			...
			<div class="grid-item">
				<table>
					...
					<tbody>
						<?php
						/* Selecciona todos los registros de la tabla producto que 
						coinciden con el idTipo_producto y el id del usuario*/
						$query="Select * From usuario where UserName like '".$_SESSION['UserName']."'"; 
						$resuser = $bd->seleccionar($query);
						$res=$resuser->fetch_assoc();
						$id=$res['idUsuario'];
						...
						cl.Cantidad, cl.idCesta FROM producto p, cesta_lineas cl WHERE p.idProducto=
						cl.idProducto AND p.idTipo_producto=". $_GET['idTipo_producto'] . 
						" AND cl.idCesta=" .$id) ;
							...
							?>
								<form action="<?= $_SERVER['PHP_SELF']; ?>" method="GET">
									<tr>
										...
										?>
										<td> <button type="submit" name="actualizarProducto" value=<?php echo  
										$producto['idProducto']; ?>>Modificar</button> </td>
										<td> <button type="submit" name="eliminarProductoProducto" value=
