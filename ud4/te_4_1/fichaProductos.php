		...
		<div class="grid-item">
			<form action="<?= $_SERVER['PHP_SELF']; ?>" method="POST">
				<ul>
					...
					<?php
					$queryidCesta="Select * From usuario WHERE Administrador = 0"; 
					$ridCesta = $bd->seleccionar($queryidCesta);
					while ($idC=$ridCesta->fetch_assoc()) {
						?>
						<input type="hidden" name="idCesta" value=
						<?php echo $idC['idUsuario']; ?>> 
